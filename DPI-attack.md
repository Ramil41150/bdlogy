...
В ByeDPI их несколько:
  ...
  * Split - нарезка по указанным позициям (позиция — байт пакета).

	Пример отправки с -s3: 1-3, 3-?.

  У нас получилось разделить его с начала пакета, но мы можем переместить это в другую часть пакета с помощью дополнительных флагов(? Я не знаю, как это назвать) про это Вы можете прочти в "Аргументы".
